...
export function NotificationCenter() {
  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen((value) => !value)} className="relative rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-white/70 transition hover:text-neon-cyan" aria-label="فتح الإشعارات">
        <Bell size={18} />
      ...
      {open && <div className="absolute right-0 top-14 z-50 w-[min(88vw,360px)]"><GlassPanel className="max-h-[70vh] overflow-y-auto border-neon-cyan/25 p-4 shadow-2xl shadow-cyan-950/30"><div className="flex items-center justify-between"><div><div className="text-[10px] uppercase tracking-[0.18em] text-neon-cyan">PulseFit</div><h3 className="mt-1 text-lg font-bold text-white">الإشعارات</h3></div><button type="button" onClick={() => setOpen(false)} className="rounded-full p-2 text-white/50 hover:text-white"><X size={16} /></button></div>{items.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-white/10 p-5 text-center text-xs text-white/45">مفيش إشعارات جديدة دلوقتي.</div> : <div className="mt-4 space-y-2">{items.map((item) => <button key={item.id} type="button" onClick={() => void readItem(item)} className={`w-full rounded-2xl border p-3 text-left transition ${item.read ? 'border-white/10 bg-white/[0.02]' : 'border-neon-cyan/25 bg-neon-cyan/5'}`}><div className="flex items-start gap-2"><div className={`mt-0.5 rounded-full p-1 ${item.read ? 'bg-white/5 text-white/40' : 'bg-neon-cyan/10 text-neon-cyan'}`}><Check size={12} /></div><div className="min-w-0 flex-1"><div className="text-xs font-bold text-white">{item.title}</div><div className="mt-1 text-xs leading-5 text-white/65">{item.message}</div><div className="mt-2 text-[9px] uppercase tracking-[0.1em] text-white/35">{new Date(item.createdAt).toLocaleString('ar-EG')}</div></div></div></button>)}</div>}</GlassPanel></div>}
    </div>
  );
}
